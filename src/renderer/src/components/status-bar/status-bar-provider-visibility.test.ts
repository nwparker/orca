...
describe('getVisibleUsageProvider', () => {
  ...
  it('hides Devin when no CLI credentials are on disk', () => {
    expect(getVisibleUsageProvider('devin', null, usageSettings())).toBe(null)
    expect(
      getVisibleUsageProvider(
        'devin',
        provider('unavailable', {
          provider: 'devin',
          error: 'Not signed in to Devin'
        }),
        usageSettings()
      )
    ).toBe(null)
  ...
  it('keeps the unavailable Devin snapshot so its reason survives instead of a pending bar', () => {
    // Why: a signed-in plan with no dated quota windows must report why rather
    // than render a fabricated 0% meter from a synthesized pending snapshot.
    const unavailable = provider('unavailable', {
      provider: 'devin',
      error: 'Devin did not report quota windows for this account'
    })
    expect(
      getVisibleUsageProvider('devin', unavailable, usageSettings({ devinAuthConfigured: true }))
    ).toBe(unavailable)
  })
